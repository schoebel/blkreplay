...
enable_slurp="${enable_slurp:-0}"

function slurp_setup
{
    (( !enable_slurp )) && return 0
    echo "$FUNCNAME slurping all data"
    for i in $(eval echo {0..$replay_max}); do
	cmd="time dd if=${replay_device[$i]} of=/dev/null bs=128k"
...
setup_list="$setup_list slurp_setup"
